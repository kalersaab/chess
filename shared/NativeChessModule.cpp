...
jsi::Value NativeChessModule::makeMove(jsi::Runtime &rt, std::string move) {
  ...
  auto executor = jsi::Function::createFromHostFunction(
      ...
      2,
      [result](jsi::Runtime &rt2,
               const jsi::Value &,
               const jsi::Value *args,
               size_t) -> jsi::Value {
        auto resolve = args[0].asObject(rt2).asFunction(rt2);
        resolve.call(rt2, jsi::String::createFromUtf8(rt2, result));
...
jsi::Value NativeChessModule::isCheckmate(jsi::Runtime &rt, bool white) {
  ...
  auto executor = jsi::Function::createFromHostFunction(
      ...
      [result](jsi::Runtime &rt2,
               ...
               size_t) -> jsi::Value {
        auto resolve = args[0].asObject(rt2).asFunction(rt2);
        resolve.call(rt2, jsi::Value(result));
        return jsi::Value::undefined();
      });
...
jsi::Value NativeChessModule::getBestMove(jsi::Runtime &rt, bool white, int depth) {
  // Run the search on a background thread so the JS event loop stays free.
  // This lets Promise resolution work correctly in release builds where
  // Hermes AOT is active and getBestMove would otherwise block the JS thread.
  auto Promise    = rt.global().getPropertyAsFunction(rt, "Promise");
        ...
        std::thread([enginePtr, invoker, white, depth, resolveVal, result]() mutable {
          *result = enginePtr->getBestMove(white, depth);
          invoker->invokeAsync([resolveVal, result](jsi::Runtime &rt3) {
            auto resolve = resolveVal->asObject(rt3).asFunction(rt3);
            resolve.call(rt3, jsi::String::createFromUtf8(rt3, *result));
          });
        }).detach();
